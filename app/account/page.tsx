...
export default function AccountPage() {
  ...
  async function saveMyInfo(values: ProfileEditValues) {
    ...
    //   '고객정보 ≠ 배송정보'로 옛 정보가 남는 사고를 막는다(단품·구독 모두, 선물·취소 제외).
    //   회원은 orders 를 직접 UPDATE 할 수 없어(RLS), 본인 주문만 갱신하는 RPC 로 처리한다.
    //   향후 배송분에 반영. best-effort — 실패해도 프로필 저장은 유지.
    if (values.address.trim()) {
      const { error: syncErr } = await getSupabase().rpc("sync_my_order_shipping", {
        p_name: values.name,
      ...
      if (syncErr) console.error("배송정보 동기화 실패:", syncErr.message);
    }
