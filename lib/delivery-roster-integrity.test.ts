...
//
//   이 파일이 지키는 불변식 4가지:
//     ① 요일 매칭은 「한 회차는 그 주를 벗어나지 않는다」 규칙을 따른다
//        (배송 시트 = 기간별 명단 = 생산 계획 = 예고 문자, 미루기·앞당김·휴배송 모두).
//     ② 총 회차는 확정된 블록 체인 합으로만 센다 — 취소된 연장분이 계속 발송되지 않는다.
//     ③ 일시정지 뒤에도 회차 예정일은 손님이 고른 배송 요일에 그대로 떨어진다.
//     ④ 결제한 회차는 공휴일·휴무 주를 지나도 빠짐없이, 중복 없이 배송된다.
