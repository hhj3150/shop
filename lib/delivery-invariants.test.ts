...
// ─────────────────────────────────────────────────────────────────────────────
// 알려진 모델 결함 — 지금은 '과배송 방어선'이 손해를 막지만, 표기·간격은 아직 어긋난다.
//   여기 적어 두는 이유: 조용히 넘어가면 다음 사람이 정상 동작으로 오해한다.
//   두 건 모두 관리자 '데이터 점검'에 해당 구독이 뜨도록 해 두었다.
// ─────────────────────────────────────────────────────────────────────────────
describe("알려진 모델 결함(재발 감시용)", () => {
  ...
  it("[결함2] 연휴가 배송 주를 넘기면 두 회차가 같은 주에 몰릴 수 있다(추석)", () => {
    // 공휴일은 '같은 주 다음 영업일'로만 미루는 규칙인데, 추석처럼 목~월 연휴면
    //   그 다음 영업일이 이미 다음 배송 주다 → 9/29(화)와 10/1(목)이 이틀 간격이 된다.
    //   회차가 사라지진 않지만 신선식품이라 배송이 몰린다. 목장 휴무처럼 '주 이월'로
    //   처리할지는 운영 정책 결정이 필요하다.
    const { dates } = simulateLifetime({ day: "thu", anchor: "2026-07-16", weeks: 12 });
